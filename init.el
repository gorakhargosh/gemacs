...
(defun goog/config/js-mode/gjslint-buffer ()
  ...
  (interactive)
  (compile (concat "gjslint --strict --unix_mode --closurized_namespaces=goog,appkit,jfk,bulletin,tvt,md " buffer-file-name)))

(defun goog/config/js-mode/gjslint-dir ()
  "Runs gjslint in strict mode on the parent directory of the file in the
current buffer."
  (interactive)
  (compile (concat "gjslint --strict --unix_mode --closurized_namespaces=goog,appkit,jfk,bulletin,tvt,md -r "
