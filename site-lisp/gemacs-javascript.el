...
(defun goog/config/js-mode/gjslint-buffer ()
  ...
  (interactive)
  (compile (concat "gjslint --strict --unix_mode --custom_jsdoc_tags=ngInject,notypecheck --closurized_namespaces=appkit,bulletin,carbon,cg,croc,goa,goog,hub,jfk,md,mountie,northstar,pqt,ooo,shub,tvt " buffer-file-name)))

(defun goog/config/js-mode/gjslint-dir ()
  "Runs gjslint in strict mode on the parent directory of the file in the
current buffer."
  (interactive)
  (compile (concat "gjslint --strict --unix_mode --custom_jsdoc_tags=ngInject,notypecheck --closurized_namespaces=appkit,bulletin,carbon,cg,croc,goa,goog,hub,jfk,md,mountie,northstar,pqt,ooo,shub,tvt -r "
